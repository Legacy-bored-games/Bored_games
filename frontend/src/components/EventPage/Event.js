  ...
  async function renderEvent() {
    ...
    setEvent((prevUser) => {
      return {
        ...
        hostedBy: (
          eventObj.creatorUser.firstName +
          ' ' +
          eventObj.creatorUser.lastName
        ).toUpperCase(),
      }
    })
  }
  ...
  return (
    <StyledEvent>
      ...
      <StyledAdditionalInfo>
        ...
        <h2>
          <i class='fa-solid fa-location-dot'></i> {event.where} | {event.when}
        </h2>
      </StyledAdditionalInfo>
      <StyledBookSeat>
        <h3>
          Seats left : {event.leftSeats} / {event.totalSeats}
        </h3>
        <StyledButton
